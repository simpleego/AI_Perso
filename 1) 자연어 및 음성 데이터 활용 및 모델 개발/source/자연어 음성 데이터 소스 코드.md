...
```markdown
뉴스 데이터 불러오기 (예: CSV 파일)
한국어 뉴스 5건을 요약해 줘. 각 기사에 대해 다음 컬럼으로 CSV만 출력해:
title, content, source, date, url
요약은 2~3문장으로, 중립적이고 간결하게. 날짜는 YYYY-MM-DD 형식.

예시
"한강공원 봄맞이 인파","주말 한강공원에 시민들이 몰리며 ...","서울일보","2025-04-12","https://..."
"국제 유가 상승","국제 유가가 배럴당 ...","연합뉴스","2025-04-12","
...
# 모델과 토크나이저 로드 (skt 공식 채널)
model_name = "skt/kobert-base-v1"
tokenizer = AutoTokenizer.from_pretrained(model_name)
model = AutoModel.from_pretrained(model_name)

# 사용 예시
text = "한국어 자연어 처리는 정말 재미있어요."
...
def get_sentence_embedding(sentence):
    ...
    # 토크나이징 (자동으로 [CLS], [SEP] 추가)
    inputs = tokenizer(sentence, return_tensors='pt', padding=True, truncation=True)

    with torch.no_grad():
        outputs = model(**inputs)

    # [CLS] 토큰의 표현을 문장 임베딩으로 사용
...
# 오디오 파일 인식
result = asr_model.transcribe_file("sample.wav")

print("=== SpeechBrain 결과 ===")
print(result)
```
