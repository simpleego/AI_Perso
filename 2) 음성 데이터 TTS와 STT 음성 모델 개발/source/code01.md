...
```mermaid
graph LR
    A["입력 오디오<br/>16kHz 모노 오디오 입력"] 
    B --> B["특징추출<br/>log-Mel 또는 MFCC 특징추출"]
    B --> C["음향 모델<br/>프레임별 음소/문자 확률 생성"]
    C --> D["언어 모델<br/>문맥 가중 적용"]
    D --> E["디코딩<br/>빈칸/중복 제거 및 비입 탐색"]
    E --> F["텍스트 후처리<br/>펴야쓰기 및 숫자/기호 정규화"]
```
